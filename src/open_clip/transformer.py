...
class VisionTransformer(nn.Module):
    ...
    def lock(self, unlocked_groups: int = 0, freeze_bn_stats: bool = False):
        """
        Freeze all parameters in the vision tower, then optionally unlock the last `unlocked_groups`
        transformer blocks (and the projection head).
        """
        # Step 1 — Freeze everything
        for p in self.parameters():
            p.requires_grad = False

        # Step 2 — Unlock the last N transformer blocks if requested
        if unlocked_groups > 0:
            total_blocks = len(self.transformer.resblocks)
            keep_last = min(unlocked_groups, total_blocks)
            start_idx = total_blocks - keep_last
            ...
            # Always unlock the final normalization + projection
            ...
            print(f"[VisionTransformer] Unlocked last {keep_last} transformer blocks "
                f"(out of {total_blocks}) + ln_post + proj.")
...
class TextTransformer(nn.Module):
    ...
    def lock(self, unlocked_layers=0, freeze_layer_norm=True):
        ...
        for i, block in enumerate(self.transformer.resblocks):
            if i < total_layers - unlocked_layers:
        ...
        if unlocked_layers == 0:
            # Freeze token embedding, positional embedding, final LayerNorm, and projection
