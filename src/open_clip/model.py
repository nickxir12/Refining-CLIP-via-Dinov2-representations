...
def lock_text_tower(self, unlocked_layers: int = 0, freeze_layer_norm: bool = True):
    # 1) lock the transformer blocks (keeps last N trainable)
    if hasattr(self.transformer, "lock"):
        self.transformer.lock(unlocked_layers, freeze_layer_norm)
    else:
        # fallback to manual if your version lacks .lock()
        blocks = getattr(self.transformer, "resblocks", getattr(self.transformer, "layers", None))
        if blocks is not None:
            total = len(blocks)
            lock_upto = max(0, total - int(unlocked_layers))
            for i, blk in enumerate(blocks):
                trainable = i >= lock_upto
                for p in blk.parameters():
                    p.requires_grad = trainable
                if trainable and not freeze_layer_norm:
    ...
    # 2) freeze embeddings & top text layers that live outside the transformer
    if hasattr(self, "token_embedding"):
        for p in self.token_embedding.parameters():
            p.requires_grad = False
    if hasattr(self, "positional_embedding") and hasattr(self.positional_embedding, "requires_grad"):
    ...
    if getattr(self, "text_projection", None) is not None:
        if isinstance(self.text_projection, nn.Linear):
            for p in self.text_projection.parameters():
                p.requires_grad = False
        elif hasattr(self.text_projection, "requires_grad"):
...
class CLIPTextCfg:
    ...
    proj_type: str = (
        "linear"  # control final text projection, 'none' forces no projection
    )
    output_tokens: bool = False
...
class CLIP(nn.Module):
    ...
    def lock_text_tower(self, unlocked_layers: int = 0, freeze_layer_norm: bool = True):
        """
        Freeze the text side of the default CLIP model.

        unlocked_layers: keep the LAST N transformer blocks trainable (0 = freeze all).
        freeze_layer_norm: if False, LayerNorm params inside unlocked blocks remain trainable.
        """

        # ---- 1) Lock the text transformer blocks ----
        # Prefer built-in .lock() if your TextTransformer implements it.
        if hasattr(self.transformer, "lock"):
            self.transformer.lock(unlocked_layers, freeze_layer_norm)
        else:
            # Fallback: manually toggle requires_grad per block
            blocks = getattr(self.transformer, "resblocks", None)
            if blocks is None:
                blocks = getattr(self.transformer, "layers", None)

            if blocks is None:
                # Unknown structure: freeze entire transformer
                for p in self.transformer.parameters():
                    p.requires_grad = False
            else:
                total = len(blocks)
                keep_last = max(0, int(unlocked_layers))
                lock_upto = max(0, total - keep_last)  # freeze [0:lock_upto), train [lock_upto:total)
                ...
                for i, blk in enumerate(blocks):
                    ...
                    # Optionally let LayerNorms stay trainable in the unlocked blocks
                    if trainable and not freeze_layer_norm:
                        for m in blk.modules():
                            if isinstance(m, nn.LayerNorm):
                                for p in m.parameters():
        ...
        # ---- 2) Freeze text-side pieces that live OUTSIDE the transformer ----
        # token embedding
        if hasattr(self, "token_embedding") and self.token_embedding is not None:
            for p in self.token_embedding.parameters():
                p.requires_grad = False

        # positional embedding is often a bare Parameter
        if hasattr(self, "positional_embedding") and self.positional_embedding is not None:
            if hasattr(self.positional_embedding, "requires_grad"):
                self.positional_embedding.requires_grad = False
        ...
        # text projection can be Linear or a bare Parameter
    ...
    def forward_intermediates(
        ...
        if output_logits:
            assert (
                image is not None and text is not None
            ), "Both image and text inputs are required to compute logits"
        ...
        if image is not None:
            ...
            if normalize and "image_features" in image_output:
                image_output["image_features"] = F.normalize(
                    image_output["image_features"], dim=-1
                )
        ...
        if text is not None:
            ...
            x, intermediates = self.transformer.forward_intermediates(
                x, attn_mask=self.attn_mask, indices=text_indices
            )
        ...
        logit_scale_exp = (
            self.logit_scale.exp() if output_logits or output_logit_scale_bias else None
        )

        if output_logits:
            image_logits = (
                logit_scale_exp * output["image_features"] @ output["text_features"].T
            )
            if self.logit_bias is not None:
                image_logits += self.logit_bias
            text_logits = image_logits.T
    ...
    def forward_intermediates(
        ...
        if text is not None:
            ...
            if normalize and "text_features" in text_output:
                text_output["text_features"] = F.normalize(
                    text_output["text_features"], dim=-1
                )
