...
from .loss import (
    ...
    SigLipLoss,
    CyCLIPLoss,
    # ClipLossWithDINORegularization,
    ClipLossWithDINOEnhancements,
    # ClipLossWithDinoWeights,
)
    ...
    **kwargs,
    ...
    if text_config.get("hf_tokenizer_name", ""):
        ...
        tn = (
            ...
            else "mc4" if "i18n" in model_name else "c4-en"
        )
...
def create_loss(args):
    if args.distill:
        ...
        # Use the modified ClipLossWithDINORegularization if lambda_dino is provided
        if args.use_CyClip:
            logging.info("Using CyClipLoss")
            
            return CyCLIPLoss(
                lambda_inmodal=getattr(args, "lambda_cyc_inmodal", 0.25),
                lambda_crossmodal=getattr(args, "lambda_cyc_crossmodal", 0.25),
                local_loss=args.local_loss,
                gather_with_grad=args.gather_with_grad,
                cache_labels=True,
                rank=args.rank,
        ...
        if args.use_dino_general:
            logging.info("Using ClipLossWithDINOEnhancements")
            return ClipLossWithDINOEnhancements(
            ...
            # Fallback to standard ClipLoss if lambda_dino is not provided or is 0
            logging.info("Using standard ClipLoss")
            return ClipLoss(
                local_loss=args.local_loss,
                gather_with_grad=args.gather_with_grad,
                cache_labels=True,
