...
def cosine_lr(optimizer, base_lr, warmup_length, steps, lr_min=0.0):
    """
    Cosine LR schedule with optional warmup and a nonzero minimum LR floor.
    lr_min is typically 0.05–0.2 * base_lr.
    """
    def _lr_adjuster(step):
        if step < warmup_length:
            lr = _warmup_lr(base_lr, warmup_length, step)
            ...
            cosine_decay = 0.5 * (1 + math.cos(math.pi * e / es))
            # keep a nonzero floor: final lr = lr_min
            lr = lr_min + (base_lr - lr_min) * cosine_decay
